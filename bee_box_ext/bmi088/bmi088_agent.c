...
#define BMI088_SPI_MAX_SPEED (10 * 1000 * 1000)

#define CSB1_Pin     BSP_IO_PORT_06_PIN_12
#define CSB2_Pin     BSP_IO_PORT_03_PIN_00
...
static rt_err_t _bmi088_get_accel_raw(struct bmi08x_dev *dev, struct bmi088_3axes *accel)
{
    ...
    lsb = buffer[1];
    msb = buffer[2];
    accel->x = (rt_int16_t)((msb << 8) | lsb); 

    lsb = buffer[3];
    msb = buffer[4];
    accel->y = (rt_int16_t)((msb << 8) | lsb);

    lsb = buffer[5];
    msb = buffer[6];
    accel->z = (rt_int16_t)((msb << 8) | lsb);
...
static rt_err_t _bmi088_get_gyro_raw(struct bmi08x_dev *dev, struct bmi088_3axes *gyro)
{
    ...
    lsb = buffer[0];
    msb = buffer[1];
    gyro->x = (rt_int16_t)((msb * 256) + lsb); 

    lsb = buffer[2];
    msb = buffer[3];
    gyro->y = (rt_int16_t)((msb * 256) + lsb); 

    lsb = buffer[4];
    msb = buffer[5];
    gyro->z = (rt_int16_t)((msb * 256) + lsb); 
...
rt_size_t bmi088_get_accel(struct bmi08x_dev *dev, struct bmi088_data *buf)
{
    ...
    buf->x = ((float)tmp.x) / 32768.0f * 6.0f * GRAVITY_EARTH;
    buf->y = ((float)tmp.y) / 32768.0f * 6.0f * GRAVITY_EARTH;
    buf->z = ((float)tmp.z) / 32768.0f * 6.0f * GRAVITY_EARTH;
...
static rt_err_t _bmi088a_init(struct bmi08x_dev *dev)
{
    ...
    struct rt_spi_device *spi_dev = (struct rt_spi_device *)(dev->accel_bus);
    _bmi088_spi_read(spi_dev, ACC_CHIP_ID_REG, 2, chip_acc_id);
...
rt_err_t bmi088a_set_meas_conf(struct bmi08x_dev *dev)
{
    ...
    if ((!is_odr_invalid) && (!is_bw_invalid) && (!is_range_invalid))
    {
        ...
        if (_bmi088_spi_read(spi_dev, ACC_CONF_REG, 2, data) == RT_EOK)
        {
            ...
            rt_thread_mdelay(10);
            _bmi088_spi_read(spi_dev, ACC_CONF_REG, 3, reg_val);
    ...
    if ((!is_odr_invalid) && (!is_range_invalid))
    {
        ...
        if (_bmi088_spi_write(spi_dev, GYRO_BANDWIDTH_REG, 1, &data) == RT_EOK)
        {
            ...
            if (_bmi088_spi_write(spi_dev, GYRO_RANGE_REG, 1, &data) == RT_EOK)
            {
                ...
                if ((reg_val[0] == 0x00) && (reg_val[1] == 0x81))
                {
                    return RT_EOK;
                }
            }
...
struct bmi08x_dev *bmi088_init(const char *acc_spi_name, const char *gyro_spi_name)
{
    ...
    {
        LOG_E("Bmi088 not support I2C temporarily");
        goto __exit;
    }
    else if (dev->accel_bus->type == RT_Device_Class_SPIDevice)
    {
        ...
        cfg.max_hz = BMI088_SPI_MAX_SPEED;
        spi_dev = (struct rt_spi_device *)dev->accel_bus;
        spi_dev->bus->owner = spi_dev;
        rt_spi_configure(spi_dev, &cfg);

        spi_dev = (struct rt_spi_device *)dev->gyro_bus;
        spi_dev->bus->owner = spi_dev;
        rt_spi_configure(spi_dev, &cfg);
    }
    else
